...
### Start up all instances of the middleware
middleware_cmd="> dstat.log; nohup dstat -cdlmnyt --output dstat.log 5 > /dev/null &
			   nohup java -jar bin/jar/ASL17_Middleware.jar -l "$(create_vm_ip ${middlewares[0]})" -p "$middleware_port" -t 2 -s false -m "$(create_vm_ip ${servers[0]})":"$memcached_port" &"
for mw_id in ${middlewares[@]}
do
	ssh $(create_vm_ip $mw_id) $middleware_cmd
done
sleep 4
echo "Started middlewares"
	...
	do
		...
		do
			...
			do
				...
				#### TODO Startup middleware here
				#### TODO Continue adapting the script
