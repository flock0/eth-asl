	...
	do
		...
		do
			middleware_cmd="> dstat.log; nohup dstat -cdlmnyt --output dstat.log 1 > /dev/null & cd asl-fall17-project;
	                        nohup java -jar dist/middleware-fchlan.jar -l "$(create_vm_ip $mw_id)" -p "$middleware_port" -t "$params_num_workers_per_mw" 
		...
		start_both_memtiers_command="> dstat.log; > ping_0.log;
					...
					--threads="$num_threads" --test-time="$single_experiment_length_sec" --expiry-range=86400-86401 --data-size=1024 --ratio="$ratio" --multi-key-get="$multiget_size" > memtier_0.log 2>&1 & 
					> ping_1.log;
					...
					--threads="$num_threads" --test-time="$single_experiment_length_sec" --expiry-range=86400-86401 --data-size=1024 --ratio="$ratio" --multi-key-get="$multiget_size" > memtier_1.log 2>&1 &"
