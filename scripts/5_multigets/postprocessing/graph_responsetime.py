...
def get_responsetime_data(sharded, multigets, middlewares, client_logfiles, reps, inputdir):
    ...
    for multiget in multigets:
        ...
        all_mt_metrics_per_rep = []
        for rep in reps:
            run_dir = os.path.join(inputdir, "{}_{}multiget".format(sharded, multiget), str(rep))
...
def get_responsetime_data_old(sharded, multigets, middlewares, client_logfiles, reps, inputdir):
    ...
    for multiget in multigets:
        ...
        for rep in reps:
            ...
            metrics = [gmws.extract_metrics_old(reqs) for reqs in concatenated_requests]
...
def plot_stacked_barchart(data, xtick_labels, worker, ax, ylim, print_first_n_bars):
    data = data.sort_values('multigets')
    ...
    ax.legend((netthreadBar[0], queuetimeBar[0], workerPreBar[0], memcachedRTTBar[0], workerPostBar[0],
               memtierResponseTimeBar[0]),
              ('Network Thread', 'Queue Time', 'Worker Preprocessing', 'Memcached RTT', 'Worker Postprocessing',
               'Latency MT<->MW'),
              loc='upper left')
    ax.set_title("{} workers".format(worker))
    ax.set_xlabel("Number of clients")
    ax.set_ylabel("Response Time (ms)")
    ax.set_ylim([0, ylim])
    #plt.xticks(x)
    ax.set_xticklabels(['dummy'] + xtick_labels[:print_first_n_bars])