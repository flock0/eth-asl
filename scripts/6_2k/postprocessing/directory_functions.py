...
def find_num_multigets(workload_dir_path):
    basedir = os.path.basename(workload_dir_path)
    start_idx = basedir.find("_") + 1 # num of multigets occures right after the underscore
    end_idx = basedir.find("multiget", start_idx)
    num_vc = int(basedir[start_idx:end_idx])
    return num_vc
