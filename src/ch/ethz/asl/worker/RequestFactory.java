...
public class RequestFactory {
	 ...
	 * @param buffer Contains the command to parse. Its position
	 * 				 must be set to the last byte of the read request
	 * @return A Request object with the parsed request
	 * @throws RequestParsingException If the command is unsupported or invalid
	 */
	public static Request createRequest(ByteBuffer buffer) throws RequestParsingException {
		...
		if(whitespaceSplit[0].equals("get")) {
			...
			if(whitespaceSplit.length < 2) {
				...
				throw new RequestParsingException(String.format("Encountered more than {} requested keys in a get request", MAX_MULTIGETS_SIZE));
			}
			
		} else if(whitespaceSplit[0].equals("set")) {
			return null; //TODO
		} else {
			throw new RequestParsingException(String.format("'{}' command is unknown.", whitespaceSplit[0]));
		}
	}
}
